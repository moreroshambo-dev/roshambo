...
import { getUser, requireUser } from "./libs/auth";
import { CURRENCY_RPS, creditBalance } from "./libs/balances";
...
const TOKEN_RATES: Record<string, number> = {
  TON: 1000,   // 1 TON = 500 game tokens
  USDT: 500,  // 1 USDT = 100 game tokens
  ...
  handler: async (ctx, args) => {
    ...
    if (isSuccess) {
      ...
      await creditBalance(ctx, {
        ...
        amount: deposit.creditedTokens,
        currency: CURRENCY_RPS
      });
    } else {
...
export const getDeposit = query({
  ...
  handler: async (ctx, args) => {
    const identity = await requireUser(ctx)
    const deposit = await ctx.db.get(args.depositId);
    if (!deposit || deposit.userId !== identity.userId) {
      return null;
    }
    return deposit;
  },
