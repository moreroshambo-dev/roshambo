...
export default function WalletPage() {
  ...
  const syncWallet = useCallback(async () => {
    if (!telegramId || !friendlyAddress) return;
    try {
      await linkWallet({ telegramId, walletAddress: friendlyAddress });
      hapticNotification("success");
      toast.success("Wallet connected successfully");
    } catch {
      toast.error("Failed to save wallet address");
    }
  }, [telegramId, friendlyAddress, linkWallet, hapticNotification]);

  useEffect(() => {
    if (wallet && friendlyAddress && telegramId) {
      if (user && user.walletAddress !== friendlyAddress) {
        syncWallet();
      }
    }
  }, [wallet, friendlyAddress, telegramId, user, syncWallet]);
