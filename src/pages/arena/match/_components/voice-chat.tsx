...
type VoiceChatProps = {
  ...
  isCreator: boolean;
  matchStatus: string;
  /** Used to detect when the actual opponent changes vs just a rematch */
  opponentId?: Id<"users">;
};
...
export default function VoiceChat({
  ...
  const signals = useQuery(
    ...
    telegramId ? { matchId, telegramId } : "skip",
  );
  ...
  const sendSignal = useCallback(
    async (type: SignalType, payload: string) => {
      ...
      try {
        await sendSignalMut({ matchId, telegramId, type, payload });
      } catch {
        // Swallow – best effort signaling
      }
    },
    [sendSignalMut, matchId, telegramId],
  );
  ...
  }, [isCreator, createPC, sendSignal, cleanup]);
  ...
  useEffect(() => {
    ...
    const processSignals = async () => {
      for (const signal of signals) {
        ...
        try {
          if (signal.type === "offer" && !isCreator) {
          ...
          } else if (signal.type === "answer" && isCreator) {
    ...
    processSignals();
  }, [signals, isActive, isCreator, sendSignal, flushIceQueue]);
  ...
  const endCall = useCallback(() => {
    ...
    setIsMuted(false);
    hapticImpact("medium");
    // Clean up signals in DB (fire and forget)
    if (telegramId) {
      clearSignalsMut({ matchId, telegramId }).catch(() => {});
    }
  }, [cleanup, hapticImpact, clearSignalsMut, matchId, telegramId]);
  ...
  // NOTE: We intentionally do NOT cleanup on matchStatus change.
      ...
      <AnimatePresence mode="wait">
        ...
        {!isActive && showInvite && (
          <motion.div
            ...
            className="w-full rounded-xl bg-gradient-to-r from-neon-purple/20 to-neon-blue/20 border border-neon-purple/40 backdrop-blur-md p-3 space-y-2.5"
          >
